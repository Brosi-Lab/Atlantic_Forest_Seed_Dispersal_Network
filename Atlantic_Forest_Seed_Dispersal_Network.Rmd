...
* Therese pointed out correctly that my vague thoughts in an email re: this test were not really coherent!
* the structure of the model at first glance seems somewhat like a $\chi^2$ test but it actually isn't:
...
# 13.7% of trees are specialists (285/777)
...
```{r model of specialist trees and higher risk frugivore associations}

# if you are a specialist, are you more likely to interact with endangered frugivores? <-- is this a different question than what the below idea gets at though if we do subset the data to only consider trees with degree = 1 I think (did a sanity check with Kaysee and she agrees)

# what about if I am asking is tree degree related to endangerment status 
# ...
# not getting anywhere
# trying a different subsetting approach - what about taking the plants that have 5 or less dispersers, since that is what we looked at in the manuscript?

plants_5_or_less_dispersers <- biome_interaction_md %>%
  filter(Plant_degree < 6, .keep_all = TRUE)

hist(plants_5_or_less_dispersers$Plant_degree) #judging from the histogram, gets rid of the big gaps in data... but I'm still going to have repeated measures of tree species interacting with more than 1 frugivore
...
```{r model validation analysis 4, eval=FALSE}

sim.out.specialist_tree_mod <- simulateResiduals(fittedModel = specialist_tree_mod, plot = T)
