...
```{r neg binomial glm of specialist trees and higher risk frugivore associations, eval = FALSE}

# trying to get the plant species' degrees and frugivore endangerment status model to run

# if you are a specialist, are you more likely to interact with endangered frugivores?

# transform plant degree into binary data
 ## doing this because other models will run but violate assumptions according to validation steps using DHARMa 

# create duplicate column to mutate into binary format

biome_interaction_md$Plant_degree_binary <- biome_interaction_md$Plant_degree

# mutate values of degree greater than 1 to 0, and degrees of 1 to 1

biome_interaction_md <- biome_interaction_md %>%
  mutate(Plant_degree_binary = ifelse(Plant_degree_binary > 1, 0, 1))

# Do I only want one row per plant species (and now we have a record of whether or not a specialist plant (degree = 1) is interacting with an endangered frugivore or not since the mutation was performed on the interaction metadata)

# ... does it even make sense to do this model?
